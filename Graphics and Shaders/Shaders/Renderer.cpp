...
void Renderer::RenderFrame() {
	...
	for (const RenderObject* object : renderObjects) {
		...
		if (objectShader != activeShader) {
			...
			int shrinkParameter = glGetUniformLocation(activeShader->GetProgramID(), "applyShrink");
			glUniform1f(shrinkParameter, applyShrink);

			int burnParameter = glGetUniformLocation(activeShader->GetProgramID(), "applyBurn");
			glUniform1f(burnParameter, applyBurn);

			int fadeParameter = glGetUniformLocation(activeShader->GetProgramID(), "applyFade");
			glUniform1f(fadeParameter, applyFade);
